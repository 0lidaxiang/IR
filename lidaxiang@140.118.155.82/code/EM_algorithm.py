...
def initialProbability():
    ...
    P_T_d1 = []
    for j in range(dNumber):
        P_T_d1.append(np.random.dirichlet(np.ones(wNumber),size=1).tolist()[0])
    P_T_d2 = []
    for j in range(dNumber):
        P_T_d2.append(np.random.dirichlet(np.ones(wNumber),size=1).tolist()[0])
    P_T_d3 = []
    for j in range(dNumber):
        P_T_d3.append(np.random.dirichlet(np.ones(wNumber),size=1).tolist()[0])
    P_T_d4 = []
    for j in range(dNumber):
        P_T_d4.append(np.random.dirichlet(np.ones(wNumber),size=1).tolist()[0])
    P_T_d5 = []
    for j in range(dNumber):
        P_T_d5.append(np.random.dirichlet(np.ones(wNumber),size=1).tolist()[0])
# -------------------- EM algorithm -----------------------------
def e_step():
...
def train():
    ...
    for i in range(1, 3):
        start = datetime.now()
        e_step()
        print("%s iteration %d, E-step excution time = %.6f"%(i,  datetime.now()-start))

        start = datetime.now()
        m_step()
        print("%s iteration %d, M-step excution time = %.6f"%(i,  datetime.now()-start))
